...
CORE_NODISCARD LinAl::Vec3dVector MGrid::calcIntersectionPoints() const
{
  ...
  const auto xDir = gridParams.xDir;
  const auto yDir = gridParams.yDir;
  const LinAl::Vec3d start = getOrigin() - xDir * gridParams.xMinLength - yDir * gridParams.yMinLength;
  ...
  CORE_POSTCONDITION_DEBUG_ASSERT(intersectionPoints.size() == intersectionPoints.capacity(), "Missing intersection points.");
...
{
  CORE_PRECONDITION_DEBUG_ASSERT(getXMax() + getXMin() > getStepWidth(), "stepWidth must be smaller than the grid length.");
  CORE_PRECONDITION_DEBUG_ASSERT(getYMax() + getYMin() > getStepWidth(), "stepWidth must be smaller than the grid length.");
...
void MGrid::calcGridSegments(Core::TVector<Geometry::Segment3d>& segments, LinAl::Vec3d dir, std::int64_t numberOfSteps) const
{
  CORE_PRECONDITION_DEBUG_ASSERT(numberOfSteps != 0, "numberOfSteps is zero.");
  CORE_PRECONDITION_DEBUG_ASSERT(Core::isEq(LinAl::norm2(dir), 1.0), "Direction must have length 1.0");

  const LinAl::Vec3d stepDir = dir * getStepWidth();
  for (std::int64_t i{0}; i < numberOfSteps; ++i)
  {
    Geometry::Segment lastSeg = segments.back();
    segments.push_back(Geometry::Segment3d{lastSeg.getSource() + stepDir, lastSeg.getTarget() + stepDir});
  }
}
} // namespace Meshler