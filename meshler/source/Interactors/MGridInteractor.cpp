...
MGridInteractor::MGridInteractor(MModel& model, FGuid defaultGridGuid)
    : m_model(model)
    , m_activeGridGuid(defaultGridGuid)
{
  auto* grid = m_model.get().find<MGrid>(defaultGridGuid);
  CORE_POSTCONDITION_DEBUG_ASSERT(grid, "Model does not contain element with the defaultGridGuid.");
  if (grid)
  {
    m_gridCopy = *grid;
...
std::optional<LinAl::Vec3d> MGridInteractor::rayIntersection(const Graphics::PickRayEvent& pickRayEvent) const
{
  const Geometry::Ray3<double_t> ray{vec3ToLinAlVec3<double_t>(pickRayEvent.origin), vec3ToLinAlVec3<double_t>(pickRayEvent.direction)};
...
bool MGridInteractor::updatedLengths(LinAl::Vec3d intersectionVec, double_t& xMin, double_t& yMin, double_t& xMax, double_t& yMax)
{
