...
namespace Meshler
{
template <typename TGeomElemContainer>
static void addTransFormedGeometryToSnapElements(SnapGeometries& snapGeometries, const TGeomElemContainer& container)
{
  for (const auto& geomElem: container)
  {
    if (geomElem.getIsSnapGeometry())
      snapGeometries.add(Geometry::transformation(geomElem.getGeometryElement(), geomElem.getTransformation()));
  }
}

SnapGeometries::SnapGeometries()
    : m_snapPlane({LinAl::Vec3d{0, 0, 0}, LinAl::Vec3d{0, 0, 1}})
{
}
SnapGeometries::SnapGeometries(const Geometry::Plane<double_t>& snapPlane)
    : m_snapPlane(snapPlane)
{
  CORE_PRECONDITION_DEBUG_ASSERT(!Core::isZero(LinAl::norm2Squared(m_snapPlane.getNormal())), "Direction with zero length is not allowed.");
}
...
void SnapGeometries::add(const Geometry::Sphere<double_t>& sphere)
{
  ...
  for (const LinAl::Vec3d& vec: {LinAl::Vec3d{0.0, 0.0, 1.0 * radius}, LinAl::Vec3d{0.0, 0.0, -1.0 * radius}})
    add(origin + vec);
}
...
void SnapGeometries::add(const GeometryElements& geometryElements)
{
  for (const auto& seg: geometryElements.getSegments())
    if (seg.getIsSnapGeometry())
      for (const Geometry::Segment3d& segment: seg.getSegments())
        add(Geometry::transformation(segment, seg.getTransformation()));

  addTransFormedGeometryToSnapElements(*this, geometryElements.getSpheres());
  addTransFormedGeometryToSnapElements(*this, geometryElements.getCylinders());
  addTransFormedGeometryToSnapElements(*this, geometryElements.getCones());
  addTransFormedGeometryToSnapElements(*this, geometryElements.getCuboids());

  for (const auto& grid: geometryElements.getGrids())
  {
    if (!grid.getIsSnapGeometry())
      continue;

    auto trafo = grid.getTransformation();

    for (const Geometry::Segment3d& segment: grid.calcGridSegments())
      add(Geometry::transformation(segment, trafo));

    for (const LinAl::Vec3d& vec: grid.calcIntersectionPoints())
      add(Geometry::transformation(vec, trafo));
  }
}
...
bool SnapGeometries::findSnapPoints(LinAl::Vec3dVector& snapPoints, const Geometry::Ray3d& placementRay) const
{
  GEOMETRY_PRECONDITION_RAY_DIRECTION_DEBUG_ASSERT(placementRay);
...
void SnapGeometries::findSnapPlane(LinAl::Vec3dVector& snapPoints, const Geometry::Ray3d& placementRay) const
{
  if (std::optional<LinAl::Vec3d> result = Geometry::intersection(m_snapPlane, placementRay))
    snapPoints.push_back(result.value());
}
std::optional<LinAl::Vec3d> SnapGeometries::findClosestSnapPoint(const LinAl::Vec3dVector& snapPoints, const Geometry::Ray3d& placementRay)
{
  LinAl::Vec3d result;
...
void SnapGeometries::addSphereSurfaceSnapPoint(LinAl::Vec3dVector& snapPoints, const Geometry::Ray3d& placementRay) const
{
  for (const Geometry::Sphere<double_t>& sphere: m_snapSpheres)
  {
    Geometry::SphereIntersection<double_t> intersection = Geometry::intersection(sphere, placementRay);
