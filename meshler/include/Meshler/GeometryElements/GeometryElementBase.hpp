...
public:
  ...
  CORE_CONSTEXPR void setIsSnapGeometry(bool isSnapGeometry) { m_isSnapGeometry = isSnapGeometry; }

  CORE_NODISCARD CORE_CONSTEXPR const LinAl::HMatrixd& getTransformation() const { return m_transformation; }
  CORE_CONSTEXPR void setTransformation(const LinAl::HMatrixd& transformation) { m_transformation = transformation; }
  ...
  CORE_CONSTEXPR void setPosition(const LinAl::Vec3d& position) { LinAl::setTranslation(m_transformation, position); }

  CORE_CONSTEXPR friend bool operator==(const Derived& lhs, const Derived& rhs) { return lhs.m_fGuid == rhs.m_fGuid && lhs == rhs; }
  CORE_CONSTEXPR friend bool operator!=(const Derived& lhs, const Derived& rhs) { return lhs.m_fGuid == rhs.m_fGuid && !(lhs == rhs); }

  CORE_CONSTEXPR friend bool operator<(const Derived& lhs, const Derived& rhs) { return lhs.m_fGuid < rhs.m_fGuid; }
  CORE_CONSTEXPR friend bool operator>(const Derived& lhs, const Derived& rhs) { return rhs < lhs; }
  CORE_CONSTEXPR friend bool operator<=(const Derived& lhs, const Derived& rhs) { return !(rhs < lhs); }
  CORE_CONSTEXPR friend bool operator>=(const Derived& lhs, const Derived& rhs) { return !(lhs < rhs); }
};
