...
public:
  ...
  {
    return std::find_if(cbegin(), cend(), [&guid](const TGeomElement& element) { return element.getFGuid() == guid; }) != cend();
  }
  ...
  {
    auto iter = std::find_if(begin(), end(), [&guid](const TGeomElement& element) { return element.getFGuid() == guid; });
