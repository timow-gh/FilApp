...
class FilAppScene {
  ...
  Core::TMap<Graphics::RenderableId, std::unique_ptr<Graphics::PointRenderable>> m_pointRenderables{};
  Core::TMap<Graphics::RenderableId, std::unique_ptr<Graphics::LineRenderable>> m_lineRenderables{};
  Core::TMap<Graphics::RenderableId, std::unique_ptr<Graphics::TriangleRenderable>> m_triangleRenderables{};
  Core::TVector<FilAppRenderable> m_filAppRenderables;

public:
  CORE_NODISCARD static FilAppScene create(filament::Engine* engine, FilAppRenderableCreator& filAppRenderableCreator);
...
private:
  ...
  addRenderable(TRenderable&& tRenderable, TRenderableMap& tRenderableMap, filament::RenderableManager::PrimitiveType primitiveType)
  {
    auto renderable = std::make_unique<TRenderable>(std::move(tRenderable));

    auto id = add(m_renderableCreator.get().createBakedColorRenderable(renderable->getVertices(), renderable->getIndices(), primitiveType));
