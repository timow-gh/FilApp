...
FilAppRenderable FilAppRenderableCreator::createBakedColorRenderable(const Core::TVector<Vertex>& vertices,
...

{
  ...
  filament::RenderableManager::Builder(1)
      ...
      .geometry(0, primitiveType, filAppRenderable.vb, filAppRenderable.ib, OFFSET, filAppRenderable.ib->getIndexCount())
      .culling(useCulling)
...
void FilAppRenderableCreator::createMaterials()
{
  ...
  mat = filament::Material::Builder()
            .package(FILAPP_RESOURCES_BAKEDVERTEXCOLOR_DATA, static_cast<size_t>(FILAPP_RESOURCES_BAKEDVERTEXCOLOR_SIZE))
            .build(*m_engine);
  matInstance = mat->createInstance();
  m_filAppMaterials.emplace(FilAppMaterialType::BAKEDVERTEXCOLOR, MatPair(mat, matInstance));

  mat = filament::Material::Builder()
            .package(FILAPP_RESOURCES_BAKEDFRAGCOLOR_DATA, static_cast<size_t>(FILAPP_RESOURCES_BAKEDFRAGCOLOR_SIZE))
            .build(*m_engine);
  matInstance = mat->createInstance();
  m_filAppMaterials.emplace(FilAppMaterialType::BAKEDFRAGCOLOR, MatPair(mat, matInstance));
}
const FilAppRenderableCreator::MatPair&
FilAppRenderableCreator::getMaterial(FilAppRenderableCreator::FilAppMaterialType filAppMaterialType) const
{
