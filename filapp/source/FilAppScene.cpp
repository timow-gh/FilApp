...
FilAppScene FilAppScene::create(filament::Engine* engine, FilAppRenderableCreator& filAppRenderableCreator)
{
...
{
  return addRenderable(std::move(renderable), m_pointRenderables, filament::RenderableManager::PrimitiveType::POINTS);
}
auto FilAppScene::add(Graphics::LineRenderable&& renderable) -> Graphics::RenderableId
{
  return addRenderable(std::move(renderable), m_lineRenderables, filament::RenderableManager::PrimitiveType::LINES);
}
auto FilAppScene::add(Graphics::TriangleRenderable&& renderable) -> Graphics::RenderableId
{
  return addRenderable(std::move(renderable), m_triangleRenderables, filament::RenderableManager::PrimitiveType::TRIANGLES);
}
...

{
  ...
  auto iter =
      std::lower_bound(m_filAppRenderables.begin(),
                       m_filAppRenderables.end(),
                       entity,
                       [](const FilAppRenderable& filAppRenderable, utils::Entity ent) { return filAppRenderable.renderableEntity < ent; });
...
Core::TVector<Graphics::RenderableId> FilAppScene::add(const Core::TVector<FilAppRenderable>& filAppRenderables)
{
